...
const loadUser = async () => {
    const result=await axios.get(`http://10.5.100.187:8082/api/v1/user/getUserId/${id}`) ;
    console.log(1);
            ...
            console.log(6);
            setLname(userData.prenom);
            console.log(6);
            setPhone(userData.mobile);
            console.log(6);
            setDate(userData.date_de_naissance);
            console.log(6);
            setCin(userData.cin);
            console.log(6);
   ...
   const handleSubmit = (event) => {
     ...
     console.log(cin);
      ...
      axios.put('http://10.5.100.187:8082/api/v1/user/update',{
               ...
               <Row>
                     ...
                     <Card>
                        <Card.Header className="d-flex justify-content-between">
                           <div className="header-title">
                              <h4 className="card-title">Modifier Un  Adhérent</h4>
                           </div>
                        </Card.Header>
                        <Card.Body>
                  ...
                  <Col xl="9" lg="8">
                     <Card>
                        <Card.Header className="d-flex justify-content-between">
                           <div className="header-title">
                              <h4 className="card-title">Informations d'Adhérent</h4>
                           </div>
                        </Card.Header>
                        <Card.Body>
                           <div className="new-user-info">
                              ...
                              />
                                          ...
                                          {provinces.map((province) => (
                                             <optgroup label={province.name}>
                                                {province.cities.map((city) => (
                                                   <option value={`${city}, ${province.name}`} selected={birthplace === `${city}, ${province.name}` ? true : false}>{city}, {province.name}</option>
                                                ))}
                                             </optgroup>
                                          ))}
                                       </select>
                                 ...
                                 <Button type="submit" variant="btn btn-primary">Ajouter Un nouveau Adhérent</Button>
